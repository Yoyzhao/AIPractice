...
def calculate_type_targets():
    ...
    if total_available == 0:
        return {'single': 0, 'multi': 0, 'judge': 0}

    # 计算各题型的目标数量（按60%:10%:30%比例）
    single_target = min(int(total_available * TYPE_RATIOS['single']), single_count)
    multi_target = min(int(total_available * TYPE_RATIOS['multi']), multi_count)
    judge_target = min(int(total_available * TYPE_RATIOS['judge']), judge_count)

    # 确保多选题至少有1道（如果有多选题的话）
    if multi_count > 0 and multi_target == 0:
        multi_target = 1
